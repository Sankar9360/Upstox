...
def token_is_valid(chat_id):
    """The DB doc merely means a token was SAVED at some point — Upstox
    access tokens always expire at 3:30 AM IST regardless, so an old doc can
    sit there looking 'connected' while every real API call 401s. This does
    one lightweight authenticated call to actually confirm it still works."""
    doc = get_token(chat_id)
    if not doc or not doc.get("access_token"):
        return False
    try:
        r = requests.get(
            "https://api.upstox.com/v2/user/profile",
            headers={"Authorization": f"Bearer {doc['access_token']}", "Accept": "application/json"},
            timeout=8,
        )
...
def send_login_reminder():
    ...
    if not (dtime(8, 30) <= current.time() <= dtime(9, 10)):
        return
    if token_is_valid(AUTHORIZED_CHAT_ID):
        return
    st = state_for(AUTHORIZED_CHAT_ID)
