...
PAPER_FUTURES_IGNORE_MARGIN = False  # Realistic ₹10,000 simulation: futures (which need ₹1.5L+ margin)
                                      # are correctly rejected/skipped instead of being fake-simulated.
                                      # Only trades that would genuinely fit the ₹10,000 capital happen —
                                      # in practice this means ATM OPTION BUYING, not futures.
...
def auto_select_mode():
    ...
    if live_quote_count == 0:
        token_msg = ""
        try:
            probe_key = next(iter(orb_map.values()))["key"]
            probe = session.get(f"{V3_URL}/market-quote/quotes", headers=headers(),
                                 params={"instrument_key": probe_key}, timeout=8)
            probe_json = probe.json() if probe.headers.get("content-type", "").startswith("application/json") else {}
            if probe.status_code == 401 or any(str(e.get("error_code", "")) == "UDAPI100050" for e in probe_json.get("errors", [])):
                token_msg = "\n🔑 Looks like the Upstox token has EXPIRED (daily 3:30 AM reset). Use /login to reconnect."
        except Exception:
            pass
        telegram_send(
    ...
    while now().time() < ENTRY_CUTOFF:
        ...
        if confirmed:
            ...
            if fut and (PAPER_MODE and PAPER_FUTURES_IGNORE_MARGIN or
                ...
                print(f"AUTO SELECTED: {best['name']} | score={best['score']} margin={margin_text}")
            else:
                mode = "OPTION"
                msg = (
                    f"🤖 AUTO SELECTED\n"
